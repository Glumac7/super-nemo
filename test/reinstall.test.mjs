...
import { INSTALL, seedUserContent, sandbox, snapshot } from "./helpers.mjs";
...
test("uninstall keeps a managed key the user changed, reports it, and reverts the rest", (t) => {
  ...
  assert.match(res.out, /kept tools\.approvalMode = "always-ask": changed since install/);
...
test("reinstall with a drifted key: exit 3 without writing, then --overwrite-drift replaces it", (t) => {
  ...
  assert.match(res.out, /modelRoles\.nemo-review = "alpha\/small:low"/);
