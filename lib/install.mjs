...
function inspect(ctx, manifest) {
  ...
  return { problems, texts, docs, symlinks, dirs };
}
...
export function buildPlan(ctx, inspected, manifest, choices, keep) {
  ...
  const prevPatterns = manifest?.config?.patterns ?? { inserted: [], createdKey: false };
  const present = patternList(doc);
  const stillThere = prevPatterns.inserted.filter((e) => {
    const i = present.findIndex((x) => equal(x, e));
    if (i !== -1) present.splice(i, 1);
    return i !== -1;
  });
  const added = insertPatterns(doc, ourPatterns(ctx), createdMaps);
  const inserted = [...stillThere, ...added.inserted];
  ...
  const nextManifest = {
    ...
    updatedAt: now,
    createdHome: manifest ? manifest.createdHome : !lstat(ctx.snHome),
    choices,
    createdDirs,
    symlinks: ctx.symlinks.map((s) => s.path),
    files: { ...(manifest?.files ?? {}) },
    config: { keys, createdMaps, patterns: { inserted, createdKey: prevPatterns.createdKey || added.createdKey } },
    blocks,
    watchdogYml,
  };
  ...
  return {
    ...
    empty: !files.length && !inspected.symlinks.length && !inspected.dirs.length && !choicesChanged && manifest?.repo === ctx.repo,
  };
}
...
export function describePlan(ctx, plan) {
  ...
  if (plan.empty) return [...out, "Nothing to change: already installed with these choices."].join("\n");
...
function apply(ctx, plan, prev, log) {
  ...
  const pending = {
    ...plan.manifest,
    status: "pending",
    createdHome: prev ? prev.createdHome : createdHome,
    txn: {
  ...
  try {
    ...
    if (createdState) fs.mkdirSync(ctx.state);
    ...
    for (const f of txnFiles.filter((x) => x.backup)) {
      ...
      copied.push(dest);
      f.backupHash = sha256(fs.readFileSync(dest));
    }
  ...
  try {
    ...
    final = { ...plan.manifest, createdHome: pending.createdHome };
...
export async function install(ctx, opts, log = console.log) {
  ...
  const defaults = computeDefaults(models, inspected.docs.config, manifest, opts);
