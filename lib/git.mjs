...
const CREDENTIALS = {
  gh: ["-c", "credential.helper=", "-c", "credential.helper=!gh auth git-credential", "-c", "core.askPass="],
  anonymous: ["-c", "credential.helper=", "-c", "core.askPass="],
};

function silentAskpass() {
  for (const dir of (process.env.PATH ?? "").split(path.delimiter).filter(Boolean)) {
    const file = path.join(dir, "true");
    try {
...
export function githubAccess(url) {
  if (!GITHUB.test(url)) return null;
  const gh = spawnSync("gh", ["auth", "status", "--hostname", "github.com"], { env: gitEnv(), stdio: "ignore" });
  return gh.status === 0 ? "gh" : "anonymous";
}

export const redact = (text) => String(text).replace(/([a-z][a-z0-9+.-]*:\/\/)[^/@\s]*@/gi, "$1***@");

function gitEnv(access) {
  ...
  if (CREDENTIALS[access]) {
    env.GIT_ASKPASS = silentAskpass();
    delete env.SSH_ASKPASS;
  }
  return env;
}

export function git(cwd, args, { access } = {}) {
  return execFileSync("git", [...(CREDENTIALS[access] ?? []), ...args], {
    cwd,
    encoding: "utf8",
    env: gitEnv(access),
