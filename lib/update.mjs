...
import { bullet, fail, noteLine, tilde } from "./ui.mjs";
...
async function verifyCurrent(ctx, branch, upstream, old, log) {
  ...
  const problems = result ? result.failures.map((f) => `  ${fail(f)}`) : ["  SUPER-NEMO is not installed."];
...
export async function update(ctx, opts, log = console.log) {
  ...
  if (old === next) {
    ...
    log(`${tilde(repo)} is at ${upstream} (${short(old)}), but the installed settings were not applied from this commit yet.`);
  ...
  const subjects = git(repo, ["log", "--format=%s", `${old}..${next}`]).split("\n");
  const shown = subjects.slice(0, 10).map((s) => `  ${bullet(s)}`);
  if (subjects.length > shown.length) shown.push(`  ...and ${subjects.length - shown.length} more`);
  const count = `${subjects.length} commit${subjects.length === 1 ? "" : "s"}`;
  log([`${dryRun ? "Would update" : "Updating"} SUPER-NEMO: ${short(old)}..${short(next)} (${count})`, ...shown].join("\n"));
  if (dryRun) {
...
function reapply(ctx, repo, commit, opts, log) {
  ...
  const flags = ["overwrite-drift", "dry-run", "verbose"].filter((f) => opts[f]).map((f) => `--${f}`);
  const res = spawnSync(process.execPath, [path.join(repo, "lib", "sn.mjs"), "install", "--reuse", "--no-smoke", ...flags], { env: ctx.env, stdio: "inherit" });
  if (res.status !== 0) {
    log(`The code is at ${short(commit)}, but re-applying the settings did not finish; fix the problem above, then run ${tilde(path.join(repo, "sn"))} update again (add --overwrite-drift to replace settings you changed)`);
    return res.status ?? 1;
  }
  return 0;
}
