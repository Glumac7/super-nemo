...
import { cleanStatus, git, redact, tryGit } from "./git.mjs";
...
function tracking(repo, clone) {
  ...
  const url = remote && remote !== "." ? tryGit(repo, ["remote", "get-url", remote]) : null;
  if (!upstream || !url) throw refuse(`branch ${branch} does not track a remote branch`);
  if (clone?.path === repo) {
  ...
  return { branch, remote, upstream, url };
}

function fetch(repo, remote, url) {
  try {
    git(repo, ["fetch", "--quiet", "--no-write-fetch-head", remote], { url });
  } catch (err) {
    const detail = redact(err.stderr ?? "").trim();
    const hint = url.startsWith("https://github.com/") ? "\nrun `gh auth login` and make sure you have access to the repository" : "";
...
export async function update(ctx, opts, log = console.log) {
  ...
  const { branch, remote, upstream, url } = tracking(repo, manifest.clone);
